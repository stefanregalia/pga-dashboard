...
year_2022_end_rankings = ['Scottie Scheffler', 'Rory McIlroy', 'Cameron Smith', 'Patrick Cantlay',
...
averages_by_month = (
    pga_2022.groupby(['month', 'player'])[['sg_ott', 'sg_app', 'sg_arg', 'sg_putt']]
    .mean().reset_index()
)
...
def update_chart(selected_metric_label, selected_month, selected_players):
    ...
    if 'Rory McIlroy' in selected_players and 'Rory McIlroy' not in filtered['player'].values:
        filtered = pd.concat([filtered, pd.DataFrame([{
            'month': selected_month, 'player': 'Rory McIlroy',
